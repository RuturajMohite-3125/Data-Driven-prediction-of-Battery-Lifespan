"""
Config sweep: 6 cycle windows × 5 GRU configs × 10 seeds.
Seeds and flags are read from .env (EOL_SEED_LIST, EOL_SHOW_PLOTS, EOL_SAVE_ARTIFACTS).

Usage:
    python run_gru_config_sweep.py
    python run_gru_config_sweep.py --configs A B --windows 0 3
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GRU sweep: cycle windows × configs × seeds"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out", default="gru_sweep_results.json",
        help="JSON output path (default: gru_sweep_results.json)"
    )
