...
_CFG_DIM_FF       = int(os.environ.get("EOL_DIM_FF",       "128"))
_CFG_DROPOUT      = float(os.environ.get("EOL_DROPOUT",    "0.3"))
_CFG_EPOCHS       = int(os.environ.get("EOL_EPOCHS",       "300"))
_CFG_BATCH_SIZE   = int(os.environ.get("EOL_BATCH_SIZE",   "4"))
_CFG_LR           = float(os.environ.get("EOL_LR",         "3e-4"))
_CFG_WEIGHT_DECAY = float(os.environ.get("EOL_WEIGHT_DECAY","5e-3"))
_CFG_LAMBDA_EOL   = float(os.environ.get("EOL_LAMBDA_EOL", "1.5"))
...
def train_model(X_train, y_train, X_val, y_val,
    ...
    if weight_decay is None: weight_decay = _CFG_WEIGHT_DECAY
    if lambda_eol  is None: lambda_eol  = _CFG_LAMBDA_EOL
    ...
    train_loader = DataLoader(TensorDataset(X_train, y_train_n),
                              batch_size=batch_size, shuffle=True)
    val_loader   = DataLoader(TensorDataset(X_val,   y_val_n),
                              batch_size=batch_size)


    model = EOLTransformer(
        num_features=X_train.size(-1), max_len=X_train.size(1),
        d_model=_CFG_D_MODEL, nhead=_CFG_NHEAD, num_layers=_CFG_NUM_LAYERS,
        dim_ff=_CFG_DIM_FF, dropout=_CFG_DROPOUT,
    ).to(DEVICE)

    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    if _CFG_SCHEDULER == "onecycle":
        sched = torch.optim.lr_scheduler.OneCycleLR(
            opt, max_lr=lr, steps_per_epoch=len(train_loader), epochs=epochs)
        _sched_per_batch = True
    else:
        sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=epochs)
        _sched_per_batch = False
    loss_fn = nn.L1Loss() if _CFG_LOSS_TYPE == "l1" else nn.SmoothL1Loss()
    ...
    for ep in range(epochs):
        ...
        for xb, yb in train_loader:
            ...
            opt.step()
            if _sched_per_batch: sched.step()
        ...
        if not _sched_per_batch: sched.step()
...
def plot_results(preds, tgts, title="Model Evaluation on Test Set"):
    ...
    ax.fill_between([lo, hi], [lo - 40, hi - 40], [lo + 40, hi + 40],
                    color="green", alpha=0.12, label="±40 band")
...
def load_cache(cache_path=CACHE_PATH):
    if not os.path.exists(cache_path):
        ...
        cells = processor.process_and_extract(cache_path=cache_path)
        return cells
...
def load_fixed_split():
    if not os.path.exists(SPLIT_FILE):
        raise FileNotFoundError(
            f"{SPLIT_FILE} not found — this script expects a curated split."
        )
    ...
    for a, b in [("train", "val"), ("train", "test"), ("val", "test")]:
        ...
        if overlap:
            print(f"WARNING: cells in both {a} and {b}: {sorted(overlap)} "
                  f"-> kept only in {a}")
...
def _aligned_proba(clf, X):
    """xgboost.XGBClassifier.predict_proba columns follow clf.classes_ order;
    align to [0, 1, 2] so missing classes don't shift columns."""
...
def train_xgb_aging_classifier(X_scalar, y, idx_tr, idx_va, idx_te, seed=42):
    ...
    Xt = X_scalar[idx_tr]; yt = y[idx_tr]
    Xv = X_scalar[idx_va]
    Xte = X_scalar[idx_te]
...
def build_cell_tensors(cells_cache, cycles=CYCLES_TO_USE):
    """Build [N, T, F] tensor from pre-extracted cache.

    Each cache entry has 'features' of shape [num_cycles, 20].
    We select rows at the requested cycle indices, zero-padding
    when a cell has fewer cycles than the index requests.
    """
    ...
    for cell in cells_cache:
        ...
        if eol < MIN_EOL_CYCLES:
            continue
        feats = cell["features"]  # [num_cycles, 20]
        n_cyc = feats.shape[0]
...
if __name__ == "__main__":
    ...
    X_tr = X_all[idx_tr]; X_va = X_all[idx_va]; X_te = X_all[idx_te]
    eol_tr = eol_all[idx_tr]; eol_va = eol_all[idx_va]; eol_te = eol_all[idx_te]
    ...
    print(f"\n[Stage 1] Tertile thresholds: "
          f"Fast EOL<={q33:.0f}, Normal ({q33:.0f},{q66:.0f}], Slow >{q66:.0f}")
    for c, name in enumerate(CLASS_NAMES):
        print(f"  train {name:<6}: {(y_all[idx_tr.numpy()] == c).sum():3d}   "
              f"val: {(y_all[idx_va.numpy()] == c).sum():3d}   "
              f"test: {(y_all[idx_te.numpy()] == c).sum():3d}")

    (_clf, oof_pred, oof_proba,
     pred_va, proba_va, pred_te, proba_te) = train_xgb_aging_classifier(
        scalar_X, y_all,
        idx_tr.numpy(), idx_va.numpy(), idx_te.numpy(),
        seed=SEED,
    )

    print("\n[Stage 1] OOF train classification report:")
    print(classification_report(y_all[idx_tr.numpy()], oof_pred,
    ...
    blend_alpha, blend_val_mae, preds_va_blend = fit_class_blend(
        preds_va, tgts_va, proba_va, class_means
    )
    ...
    print(f"\n{'cell':<10} {'true':>6} {'pred':>7} {'|err|':>7} "
          f"{'true_cls':>9} {'pred_cls':>9}  probs[F,N,S]")
    ...
    for i, name in enumerate([names[j] for j in idx_te.tolist()]):
        ...
        p = proba_te[i]
        print(f"{name:<10} {tgts_te[i]:6.0f} {preds_te_blend[i]:7.1f} "
    ...
    if SAVE_ARTIFACTS:
        ckpt_path = "best_eol_transformer_50_cycle_window.pt"
        torch.save({
            "model_state_dict": model.state_dict(),
            "model_config": {
                "num_features": X_tr.size(-1),
                "max_len": X_tr.size(1),
                "d_model": 64, "nhead": 4, "num_layers": 2,
                "dim_ff": 128, "dropout": 0.3,
            },
