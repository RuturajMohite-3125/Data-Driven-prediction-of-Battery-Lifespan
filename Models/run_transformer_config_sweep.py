"""
Config sweep for the two-stage EOL Transformer.
...
Usage:
    python run_transformer_config_sweep.py                      # voltage ablation, all windows/configs
    python run_transformer_config_sweep.py --configs A B --windows V0 V4 all
    python run_transformer_config_sweep.py --mode cycles --windows 0 3
...
def _run_one(seed: int, cfg: dict, cycles: list, dqdv_window="all") -> dict:
    ...
    env_keys = (
        ...
        "EOL_LOSS_TYPE", "EOL_SCHEDULER", "EOL_DQDV_WINDOW",
    )
...
def _print_global_table(all_results):
    ...
    print(f"{'#'*72}")
    hdr = f"{'ID':<10} {'Window':<44} {'Config':<28} {'Acc%':>12} {'MAE':>10} {'RMSE':>10} {'R²':>8}"
    print(hdr)
    print("─" * len(hdr))
...
def _plot_grid_heatmap(all_results, out_path):
    ...
    ax.set_title("Best-config Test MAE per (input × voltage) window")
...
def _plot_bar(all_results, out_path):
    ...
    ax.set_title("Sweep: Test MAE per window × config")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transformer sweep: window (voltage or cycle) × configs × seeds"
    )
    parser.add_argument(
    ...
    if args.mode == "grid":
        ...
        out_path = args.out or str(Path(__file__).parent / "transformer_grid_ablation_results.json")
    elif args.mode == "voltage":
        runs = build_voltage_runs(volt_keys=args.windows)
        out_path = args.out or str(Path(__file__).parent / "window_ablation_results.json")
    else:  # cycles
        runs = build_cycle_runs(input_idxs=args.input_windows)
        out_path = args.out or str(Path(__file__).parent / "transformer_sweep_results.json")
