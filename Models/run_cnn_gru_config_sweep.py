"""
Config sweep for the two-stage EOL CNN-GRU.
...
Knobs varied per config:
  ...
  GRU  : gru_hidden_size, num_layers, rnn_dropout (auto when layers>1)
  Head : head_dropout
...
Usage:
    python run_cnn_gru_config_sweep.py                             # grid (default)
    python run_cnn_gru_config_sweep.py --mode voltage --windows V0 V4 all
    python run_cnn_gru_config_sweep.py --mode grid --input-windows 3 --windows V5 all
    python run_cnn_gru_config_sweep.py --mode cycles --input-windows 0 3
"""
...
# gru_hidden   : bidirectional GRU hidden size (output = gru_hidden*2)
# num_layers   : GRU stacked layers (recurrent dropout=0.3 when >1)
# cnn_dropout  : dropout after each conv block
# head_dropout : dropout in the MLP prediction head
CONFIGS = {
    "A": {
        "label": "A — Baseline (cnn=64, gru=128, layers=1)",
        "model": dict(cnn_channels=64,  gru_hidden=128, num_layers=1, cnn_dropout=0.2, head_dropout=0.3),
        "train": dict(epochs=300, batch_size=4, lr=3e-4, weight_decay=5e-3, lambda_eol=1.5),
        "loss": "smooth_l1", "scheduler": "cosine",
    },
    "B": {
        "label": "B — Wider CNN + Deeper GRU (cnn=128, gru=128, layers=2)",
        "model": dict(cnn_channels=128, gru_hidden=128, num_layers=2, cnn_dropout=0.2, head_dropout=0.3),
        "train": dict(epochs=300, batch_size=8, lr=2e-4, weight_decay=5e-3, lambda_eol=1.5),
        "loss": "smooth_l1", "scheduler": "cosine",
    },
    "C": {
        "label": "C — Narrow CNN + Wide GRU (cnn=32, gru=256, layers=1)",
        "model": dict(cnn_channels=32,  gru_hidden=256, num_layers=1, cnn_dropout=0.1, head_dropout=0.2),
        "train": dict(epochs=300, batch_size=4, lr=3e-4, weight_decay=1e-2, lambda_eol=1.5),
        "loss": "smooth_l1", "scheduler": "cosine",
    },
    "D": {
        "label": "D — OneCycleLR (cnn=64, gru=128, layers=2)",
        "model": dict(cnn_channels=64,  gru_hidden=128, num_layers=2, cnn_dropout=0.2, head_dropout=0.3),
        "train": dict(epochs=300, batch_size=4, lr=5e-4, weight_decay=5e-3, lambda_eol=1.0),
        "loss": "smooth_l1", "scheduler": "onecycle",
    },
    "E": {
        "label": "E — L1 Loss + Small (cnn=32, gru=64, layers=2)",
        "model": dict(cnn_channels=32,  gru_hidden=64,  num_layers=2, cnn_dropout=0.3, head_dropout=0.4),
...
def _run_one(seed: int, cfg: dict, cycles: list, dqdv_window="all") -> dict:
    ...
    env_keys = (
        ...
        "EOL_CNN_CHANNELS", "EOL_GRU_HIDDEN", "EOL_NUM_LAYERS",
        "EOL_CNN_DROPOUT", "EOL_HEAD_DROPOUT",
    ...
    os.environ["EOL_CNN_CHANNELS"]   = str(model_cfg["cnn_channels"])
    os.environ["EOL_GRU_HIDDEN"]     = str(model_cfg["gru_hidden"])
    os.environ["EOL_NUM_LAYERS"]     = str(model_cfg["num_layers"])
    os.environ["EOL_CNN_DROPOUT"]    = str(model_cfg["cnn_dropout"])
    ...
    try:
        with warnings.catch_warnings():
            ...
            with redirect_stdout(buf), redirect_stderr(buf):
                runpy.run_path(
                    str(Path(__file__).parent / "CNN_GRU_EOL.py"),
                    run_name="__main__",
                )
    finally:
...
def _plot_grid_heatmap(all_results, out_path):
    ...
    ax.set_title("CNN-GRU: best-config Test MAE per (input × voltage) window")
...
def _plot_bar(all_results, out_path):
    ...
    ax.set_title("CNN-GRU sweep: Test MAE per window × config")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="CNN-GRU sweep: window (grid / voltage / cycle) × configs × seeds"
    )
    parser.add_argument(
    ...
    if args.mode == "grid":
        ...
        out_path = args.out or str(Path(__file__).parent / "cnn_gru_grid_ablation_results.json")
    elif args.mode == "voltage":
        runs = build_voltage_runs(volt_keys=args.windows)
        out_path = args.out or str(Path(__file__).parent / "cnn_gru_window_ablation_results.json")
    else:  # cycles
        runs = build_cycle_runs(input_idxs=args.input_windows)
        out_path = args.out or str(Path(__file__).parent / "cnn_gru_sweep_results.json")
