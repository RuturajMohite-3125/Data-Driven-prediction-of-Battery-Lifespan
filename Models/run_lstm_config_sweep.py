"""
Config sweep: 6 cycle windows × 5 LSTM configs × 10 seeds.
Seeds and flags are read from .env (EOL_SEED_LIST, EOL_SHOW_PLOTS, EOL_SAVE_ARTIFACTS).

Usage:
    python run_lstm_config_sweep.py
    python run_lstm_config_sweep.py --configs A B --windows 0 3
...
def run_sweep(config_keys: list, window_indices: list, out_path: str):
    ...
    print(f"{'#'*72}")
    hdr = f"{'ID':<8} {'Window':<44} {'Config':<34} {'Acc%':>12} {'MAE':>10} {'RMSE':>10} {'R²':>8}"
    print(hdr)
    print("─" * len(hdr))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LSTM sweep: cycle windows × configs × seeds"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out", default="lstm_sweep_results.json",
        help="JSON output path (default: lstm_sweep_results.json)"
    )
