"""
Config sweep for the two-stage EOL LSTM.
...
Two sweep modes (choose with --mode):
  ...
  grid     (default) — cross product of dQ/dV VOLTAGE windows × input (cycle)
             ...
             configs over all seeds. Shows which (cycle window, voltage band)
             combination is most EOL-predictive.
...
All modes: configs × seeds per window-unit, Train/Val/Test summarised as mean ± std.
Seeds/flags come from .env (EOL_SEED_LIST, EOL_SHOW_PLOTS, EOL_SAVE_ARTIFACTS).

Usage:
    python run_lstm_config_sweep.py                                # grid (default)
    python run_lstm_config_sweep.py --mode voltage --windows V0 V4 all
    python run_lstm_config_sweep.py --mode grid --input-windows 3 --windows V5 all
    python run_lstm_config_sweep.py --mode cycles --input-windows 0 3
    python run_lstm_config_sweep.py --out results.json
"""
    ...
    try:
        with warnings.catch_warnings():
            ...
            with redirect_stdout(buf), redirect_stderr(buf):
                runpy.run_path(
                    str(Path(__file__).parent / "LSTM_seq2seq.py"),
                    run_name="__main__",
                )
    finally:
...
def _plot_grid_heatmap(all_results, out_path):
    ...
    ax.set_title("LSTM: best-config Test MAE per (input × voltage) window")
...
def _plot_bar(all_results, out_path):
    ...
    ax.set_title("LSTM sweep: Test MAE per window × config")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="LSTM sweep: window (grid / voltage / cycle) × configs × seeds"
    )
    parser.add_argument(
    ...
    if args.mode == "grid":
        ...
        out_path = args.out or str(Path(__file__).parent / "lstm_grid_ablation_results.json")
    elif args.mode == "voltage":
        runs = build_voltage_runs(volt_keys=args.windows)
        out_path = args.out or str(Path(__file__).parent / "lstm_window_ablation_results.json")
    else:  # cycles
        runs = build_cycle_runs(input_idxs=args.input_windows)
        out_path = args.out or str(Path(__file__).parent / "lstm_sweep_results.json")
